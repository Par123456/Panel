...
let userID = '2fef41cd-5ba7-47cc-a238-395d54d29042';
                        ...
                        if (request.method === 'POST' && request.headers.get('content-type') === 'application/json') {
                            try {
                                ...
                                if (warpPlusError) {
                                    ...
                                    return new Response('VPN configs updated successfully', { status: 200 });
                                }
                            } catch (error) {
                                console.log(error);
            ...
            <tr>
                <td>
                    ${config.address === 'Best-Ping' 
                        ? `<div  style="justify-content: center;"><span><b>💦 NOQ F - Best-Ping 💥</b></span></div>` 
                        : config.address === 'WorkerLess'
                            ? `<div  style="justify-content: center;"><span><b>💦 NO1 F - WorkerLess ⭐</b></span></div>`
                            : config.address === 'Best-Fragment'
                                ? `<div  style="justify-content: center;"><span><b>💦 NO1 F - Best-Fragment 😎</b></span></div>`
                                : config.address
                    }
                </td>
                <td>
			...
			<div class="table-container">
				<table id="warp-pro-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/warpsub/${userID}?app=nikang#NO1-VPN-Pro', 'Warp Pro Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=nikang#NO1-VPN-Pro', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
					</tr>
					<tr>
						...
						<td>
                            <button onclick="openQR('sing-box://import-remote-profile?url=https://${hostName}/warpsub/${userID}?app=hiddify#NO1-VPN-Pro', 'Warp Pro Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=hiddify#NO1-VPN-Pro', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
					</tr>
...
async function getXrayWarpConfigs (env, client) {
    ...
    xrayWarpBestPing.remarks = client === 'nikang' ? '💦 NO1 - VPN Pro Best Ping 🚀' : '💦 NO1 - Warp Best Ping 🚀';
    ...
    xrayWarpOutbounds.forEach((outbound, index) => {
        xrayWarpConfigs.push({
            ...xrayWarpConfig,
            remarks: client === 'nikang' ? `💦 NO1 - VPN Pro ${index + 1} 🇮🇷` : `💦 NO1 - VPN ${index + 1} 🇮🇷`,
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag.includes('warp-out')) {
            ...
            xrayWoWConfig.remarks = client === 'nikang' ? `💦 NO1 - WoW Pro ${index/2 + 1} 🌍` : `💦 NO1 - WoW ${index/2 + 1} 🌍`;
    ...
    xrayWoWBestPing.remarks = client === 'nikang' ? '💦 NO1 - WoW Pro Best Ping 🚀' : '💦 NO1 - WoW Best Ping 🚀';
